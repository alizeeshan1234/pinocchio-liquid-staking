...
pub fn process_enable_auto_compound(accounts: &[AccountInfo], instruction_data: &[u8]) -> ProgramResult {
    let [
        user,
        user_stake_account,
    ] = accounts else {
    ...
    if !position_found {
        return Err(ProgramError::Custom(1002)); // Position not found
    }
